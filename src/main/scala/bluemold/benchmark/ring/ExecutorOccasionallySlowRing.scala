...
object ExecutorOccasionallySlowRing {
  ...
  def main( args: Array[String] ) {
    ...
    println( "***** Benchmark: Occasionally Slow Ring - BlueMold Executor ( Simple )" )
    println( "Number of Actors = " + numNodes.formatted( "%,d" ) )
    println( "Number of Messages = " + ( numNodes * numMsgs).formatted( "%,d" ) )

    val myActor = new ExecutorOccasionallySlowRing( strategy ).start()
...
class ExecutorOccasionallySlowRing( _strategy: ActorStrategy ) extends SimpleActor()( _strategy ) {
  import ExecutorOccasionallySlowRing._
    ...
    msg match {
      case count: Int => {
        if ( count > 0 ) {
          ...
          else nextActor = new ExecutorOccasionallySlowRing( getNextStrategy() ).start()
          nextActor ! ( count - 1 )
        } else {
