...
class StickyRegisteredRing extends RegisteredActor {
  import StickyRegisteredRing._
  ...
  protected def react = {
      case count: Int => {
        if ( count > 0 ) {
          ...
          else nextActor = actorOf( new StickyRegisteredRing() ).start()
          nextActor ! ( count - 1 )
        } else {
