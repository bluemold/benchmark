...
class OccasionallySlowRing( _strategy: ActorStrategy ) extends SimpleActor()( _strategy ) {
  import OccasionallySlowRing._
    ...
    msg match {
      case count: Int => {
        if ( count > 0 ) {
          ...
          else nextActor = new OccasionallySlowRing( getNextStrategy() ).start()
          nextActor ! ( count - 1 )
        } else {
