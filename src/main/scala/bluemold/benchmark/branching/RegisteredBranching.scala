...
object RegisteredBranching {
  ...
  def main( args: Array[String] ) {
    println( "***** Benchmark: Branching - BlueMold ( Registered )" )
    ...
    val myActor = actorOf( new RegisteredBranching() ).start()
...
class RegisteredBranching extends RegisteredActor {
  import RegisteredBranching._
  ...
  protected def react = {
      case ( "create", parent: ActorRef, count: Int ) => {
        ...
        if ( count > 0 ) {
          ...
          else leftActor = actorOf( new RegisteredBranching ).start()
          if ( rightActor != null )
            throw new RuntimeException( "if already created the right actor" );
          else rightActor = actorOf( new RegisteredBranching ).start()
