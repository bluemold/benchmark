...
object StickySimpleBranching {
  ...
  def main( args: Array[String] ) {
    ...
    println( "***** Benchmark: Branching - BlueMold Sticky ( Simple )" )
    ...
    val myActor = new StickySimpleBranching().start()
...
class StickySimpleBranching( implicit _strategy: ActorStrategy ) extends SimpleActor()( _strategy ) {
  import StickySimpleBranching._
    ...
    msg match {
      case ( "create", parent: ActorRef, count: Int ) => {
        ...
        if ( count > 0 ) {
          ...
          else leftActor = new StickySimpleBranching()(getNextStrategy()).start()
          if ( rightActor != null )
            throw new RuntimeException( "if already created the right actor" );
          else rightActor = new StickySimpleBranching()(getNextStrategy()).start()
